...
------------
 := by
  ...
  · exact Eq.symm (Nat.mod_eq_of_lt <| lt_trans hi (Nat.lt_add_one _))
  · exact Eq.symm (Nat.mod_eq_of_lt <| lt_trans hi (Nat.lt_add_one _))
  · exact Eq.symm (Nat.mod_eq_of_lt <| Nat.succ_lt_succ hi)
...
:= by
  ...
  · intro F1 F2 h1 h2
    ...
    have := t2.symm ▸ (instUniqueHarderNarasimhanFiltration.uniq HN1)
